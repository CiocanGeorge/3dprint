...
function toDB(form, userId) {
  return {
    ...
    total_ron: Number(form.totalRON) || 0,
    status: form.status,
...
export function OrdersPage() {
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .then(({ data, error }) => {
        if (!error) setOrders(data.map(fromDB));
  ...
  const openAdd = (defaultType) => {
    setForm({ ...EMPTY_FORM, type: defaultType || "supplier" });
    setCurrent(null);
    setModal("add");
  };
  const openEdit = (o) => {
  ...
  const handleSave = async () => {
    ...
    if (current) {
      ...
      if (!error)
        setOrders((os) =>
          os.map((o) => (o.id === current.id ? fromDB(data) : o)),
        );
    } else {
      const { data, error } = await supabase
        .from("orders")
        ...
        .select()
        .single();
      if (!error) setOrders((os) => [fromDB(data), ...os]);
    }
    setSaving(false);
    setModal(null);
  };
    ...
    <div className={styles.page}>
      ...
      <div className={styles.statsGrid}>
        ...
        <StatCard
          ...
          label="În așteptare"
          value={pending}
          color={pending > 0 ? "orange" : "green"}
        />
        <StatCard
          icon="💰"
          label="Venituri livrate"
          value={`${revenueRON.toLocaleString()} RON`}
          color="green"
        />
        <StatCard
          icon="🛒"
          label="Cheltuieli livrate"
          value={`${spendRON.toLocaleString()} RON`}
          color="red"
        />
      </div>
      ...
      <div className={styles.toolbar}>
        <div className={styles.searchWrap}>
          ...
          <input
            ...
            placeholder="Caută furnizor, client, descriere..."
            value={search}
          ...
          {filtered.map((o) => (
            <div key={o.id} className={styles.row}>
              <div className={styles.rowLeft}>
                <div
                  className={`${styles.typeChip} ${styles["type_" + o.type]}`}
                >
                ...
                <div className={styles.rowInfo}>
                  ...
                  {o.description && (
                    <div className={styles.rowMeta}>{o.description}</div>
                  )}
                </div>
              </div>
              <div className={styles.rowRight}>
                {o.totalRON > 0 && (
                  <span className={styles.price}>
                    {Number(o.totalRON).toLocaleString()} RON
                  </span>
                )}
                <span
                  className={`${styles.statusBadge} ${styles["status_" + STATUS_COLORS[o.status]]}`}
                >
                  {STATUS_LABELS[o.status]}
                </span>
                <span className={styles.rowDate}>
                  {new Date(o.orderDate).toLocaleDateString("ro-RO")}
                </span>
                <button className={styles.editBtn} onClick={() => openEdit(o)}>
                  <EditIcon />
                </button>
              </div>
            </div>
      ...
      {(modal === "add" || modal === "edit") && (
        <Modal
          title={modal === "add" ? "Adaugă comandă" : "Editează comandă"}
          ...
          <div className={styles.formGrid}>
            ...
            <Field label="Total (RON)">
              <input
                ...
                step="0.01"
                value={form.totalRON}
                onChange={(e) => setF("totalRON")(e.target.value)}
                placeholder="0.00"
              />
            </Field>
            <Field label="Status">
            ...
            <Field label="Note" wide>
              <textarea
                ...
                placeholder="Link comandă, număr AWB..."
                rows={3}
              />
            </Field>
          </div>
