...
const FEATURES = [
  { icon: '🧵', title: 'Stoc filamente', desc: 'Urmărește fiecare rolă — cantitate, culoare, brand. Alerte automate când stocul scade.' },
  { icon: '🖨️', title: 'Jurnal imprimări', desc: 'Înregistrează fiecare imprimare cu filamentul consumat, durata și statusul.' },
  { icon: '📦', title: 'Gestiune comenzi', desc: 'Comenzi de la clienți și furnizori într-un singur loc, cu statusuri și valori.' },
  { icon: '🧮', title: 'Calculator costuri', desc: 'Calculează instant prețul corect: filament + energie + manoperă + profit.' },
  { icon: '📊', title: 'Statistici', desc: 'Grafice clare despre consum, cheltuieli și venituri în timp.' },
  { icon: '☁️', title: 'Sincronizare cloud', desc: 'Datele tale sunt salvate în cloud. Accesezi de pe orice dispozitiv.' },
];

const PLANS = [
  {
    name: 'Free', price: '0', sub: 'pentru totdeauna',
  ...
  {
    name: 'Pro', price: '39', sub: 'RON / lună',
    ...
    cta: 'Încearcă Pro', highlight: true,
  },
];
...
function Nav({ width }) {
  ...
  return (
    <nav style={{
      ...
      ...(scrolled ? { background: 'rgba(250,250,250,0.92)', backdropFilter: 'blur(12px)', borderBottom: '1px solid #e2e8f0', boxShadow: '0 1px 20px rgba(0,0,0,0.06)' } : {}),
    }}>
      <div style={{ maxWidth: 1100, margin: '0 auto', height: 64, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <span style={{ width: 32, height: 32, background: '#0f172a', color: '#fff', borderRadius: 8, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, fontSize: 14, flexShrink: 0 }}>3D</span>
          <span style={{ fontWeight: 700, fontSize: 15, letterSpacing: '-0.3px' }}>3D•Print Studio</span>
        </div>
        {mob ? (
          <button onClick={() => setMenuOpen(o => !o)} style={{ background: 'none', border: 'none', fontSize: 22, cursor: 'pointer', color: '#0f172a', padding: 4 }}>
            {menuOpen ? '✕' : '☰'}
          </button>
        ) : (
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            <a href="#features" style={{ fontSize: 14, color: '#64748b', textDecoration: 'none', padding: '8px 12px', borderRadius: 8 }}>Features</a>
            {/* <a href="#pricing" style={{ fontSize: 14, color: '#64748b', textDecoration: 'none', padding: '8px 12px', borderRadius: 8 }}>Prețuri</a> */}
            <a href="/login" style={{ fontSize: 14, color: '#64748b', textDecoration: 'none', padding: '8px 12px', borderRadius: 8 }}>Login</a>
            <a href="/register" style={{ fontSize: 14, background: '#0f172a', color: '#fff', textDecoration: 'none', padding: '9px 20px', borderRadius: 10, fontWeight: 600 }}>Începe gratuit</a>
          </div>
        )}
      </div>
      {mob && menuOpen && (
        <div style={{ background: '#fff', borderTop: '1px solid #f1f5f9', padding: '12px 0 16px', display: 'flex', flexDirection: 'column', gap: 4 }}>
          {['Features', 'Prețuri', 'Login'].map(item => (
            <a key={item} href="#" onClick={() => setMenuOpen(false)} style={{ fontSize: 15, color: '#475569', textDecoration: 'none', padding: '10px 20px', fontWeight: 500 }}>{item}</a>
          ))}
          <div style={{ padding: '8px 20px 0' }}>
            <a href="#" style={{ display: 'block', textAlign: 'center', background: '#0f172a', color: '#fff', textDecoration: 'none', padding: '12px', borderRadius: 12, fontWeight: 700, fontSize: 15 }}>Începe gratuit →</a>
          </div>
        </div>
      )}
    </nav>
  ...
  return (
    <div style={{
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontSize: 12, fontWeight: 600, color: '#0f172a', marginBottom: 6 }}>{brand} <span style={{ color: '#94a3b8', fontWeight: 400 }}>{material}</span></div>
      ...
      {low && (
        <span style={{ position: 'absolute', top: -8, right: 10, background: '#fef2f2', color: '#ef4444', fontSize: 10, fontWeight: 600, padding: '2px 8px', borderRadius: 99, border: '1px solid #fecaca' }}>⚠ Scăzut</span>
      )}
    </div>
  );
}
...
function Hero({ width }) {
  ...
  return (
    <section ref={ref} style={{
      ...
      <div style={{ maxWidth: showCards ? 560 : '100%', position: 'relative', zIndex: 1 }}>
        <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: '#f1f5f9', border: '1px solid #e2e8f0', borderRadius: 99, padding: '6px 14px', fontSize: 12, fontWeight: 500, color: '#475569', marginBottom: 28, ...trans(0) }}>
        ...
        <h1 style={{ fontSize: mob ? 36 : 'clamp(38px,5vw,60px)', fontWeight: 800, lineHeight: 1.1, letterSpacing: '-1.5px', margin: '0 0 24px', color: '#0f172a', ...trans(0.1) }}>
          Controlul complet<br />
          <span style={{ color: '#5c6ac4' }}>al studioului tău</span><br />
          de imprimare 3D.
        </h1>

        <p style={{ fontSize: mob ? 15 : 17, lineHeight: 1.7, color: '#64748b', margin: '0 0 36px', maxWidth: 460, ...trans(0.2) }}>
          3D•Print Studio te ajută să gestionezi stocul de filamente, să urmărești imprimările, să calculezi costuri corecte și să ții evidența comenzilor — totul într-un singur loc.
        </p>

        <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', ...trans(0.3) }}>
          <a href="#" style={{ background: '#0f172a', color: '#fff', textDecoration: 'none', padding: mob ? '12px 24px' : '14px 28px', borderRadius: 12, fontWeight: 700, fontSize: mob ? 14 : 15 }}>Începe gratuit →</a>
          <a href="#" style={{ background: 'transparent', color: '#0f172a', textDecoration: 'none', padding: mob ? '12px 20px' : '14px 24px', borderRadius: 12, fontWeight: 600, fontSize: mob ? 14 : 15, border: '1.5px solid #e2e8f0' }}>Am deja cont</a>
        </div>

        <p style={{ fontSize: 12, color: '#94a3b8', marginTop: 16, ...trans(0.4) }}>Fără card de credit. Contul free nu expiră niciodată.</p>

        {/* Mobile filament cards — horizontal scroll */}
        {!showCards && (
          <div style={{ marginTop: 40, overflowX: 'auto', display: 'flex', gap: 12, paddingBottom: 8, ...trans(0.5) }}>
            {[
              { color: '#e03e3e', brand: 'Bambu Lab', material: 'PLA', remaining: 780, weight: 1000 },
              { color: '#3b82f6', brand: 'eSUN', material: 'PETG', remaining: 320, weight: 1000 },
              { color: '#22c55e', brand: 'Prusament', material: 'PLA+', remaining: 55, weight: 1000, low: true },
              { color: '#f59e0b', brand: 'Fiberlogy', material: 'ABS', remaining: 900, weight: 1000 },
            ].map((f, i) => (
      ...
      {showCards && (
        <div style={{
          ...
          {[
            ...
            { color: '#22c55e', brand: 'Prusament', material: 'PLA+', remaining: 55, weight: 1000, low: true },
            { color: '#f59e0b', brand: 'Fiberlogy', material: 'ABS', remaining: 900, weight: 1000 },
          ].map((f, i) => <FilamentCard key={i} {...f} delay={i * 0.1} />)}
        </div>
      )}
    </section>
  );
...
function Features({ width }) {
  ...
  return (
    <section id="features" ref={ref} style={{ padding: mob ? '70px 20px' : '100px 40px', background: '#fff' }}>
      <div style={{ maxWidth: 1100, margin: '0 auto' }}>
        <div style={{ textAlign: 'center', marginBottom: mob ? 40 : 60 }}>
          <h2 style={{ fontSize: mob ? 28 : 'clamp(28px,4vw,44px)', fontWeight: 800, letterSpacing: '-1px', lineHeight: 1.15, margin: '0 0 16px', opacity: inView ? 1 : 0, transform: inView ? 'none' : 'translateY(20px)', transition: 'all 0.6s ease' }}>
            Tot ce ai nevoie,<br />nimic în plus.
          </h2>
          <p style={{ fontSize: mob ? 14 : 16, color: '#64748b', maxWidth: 480, margin: '0 auto', lineHeight: 1.7, opacity: inView ? 1 : 0, transition: 'all 0.6s ease 0.1s' }}>
            Construit specific pentru maker-ii care imprimă la comandă și vor să știe exact cât costă fiecare piesă.
          </p>
        </div>
        <div style={{
          display: 'grid',
          gridTemplateColumns: mob ? '1fr' : width < 900 ? 'repeat(2, 1fr)' : 'repeat(3, 1fr)',
          gap: mob ? 14 : 24,
        }}>
          {FEATURES.map((f, i) => (
            <div key={f.title} style={{
              background: '#fafafa', border: '1px solid #f1f5f9', borderRadius: 16,
...
const SLIDES = [
  ...
  { tag: 'Filamente', title: 'Stocul tău, mereu la zi', desc: 'Adaugă role noi, urmărești cantitatea rămasă și primești alerte când se apropie de zero.', color: '#e03e3e', mock: 'filaments' },
  ...
  return (
    <Shell active="Overview">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 10, marginBottom: 16 }}>
        <SC v="12" label="Total role" color="#5c6ac4" /><SC v="8.4kg" label="Disponibil" color="#8a6bbf" /><SC v="2" label="Stoc scăzut" color="#ef4444" /><SC v="5" label="Branduri" color="#3b82f6" />
      </div>
      ...
      {[{ brand: 'Bambu Lab', mat: 'PLA', color: '#e03e3e', pct: 78 }, { brand: 'eSUN', mat: 'PETG', color: '#3b82f6', pct: 32 }, { brand: 'Prusament', mat: 'PLA+', color: '#22c55e', pct: 5, low: true }].map(f => (
        <MRow key={f.brand}>
          ...
          <div style={{ flex: 1 }}><div style={{ fontSize: 10, fontWeight: 600 }}>{f.brand} · {f.mat}</div><MBar pct={f.pct} color={f.color} low={f.low} /></div>
  ...
  return (
    <Shell active="Filamente">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
        {[{ brand: 'Bambu Lab', mat: 'PLA', colorName: 'Fire Red', color: '#e03e3e', rem: 780, w: 1000, price: 89 }, { brand: 'eSUN', mat: 'PETG', colorName: 'Ocean Blue', color: '#3b82f6', rem: 320, w: 1000, price: 65 }, { brand: 'Prusament', mat: 'PLA+', colorName: 'Galaxy Green', color: '#22c55e', rem: 55, w: 1000, price: 110, low: true }, { brand: 'Fiberlogy', mat: 'ABS', colorName: 'Solar Yellow', color: '#f59e0b', rem: 900, w: 1000, price: 72 }].map(f => (
...
function MockPrints() {
  return (
    <Shell active="Imprimări">
      ...
      {[{ name: 'Vază decorativă', mat: 'PLA Roșu', g: 48, dur: '2h 15min', status: 'finalizat', color: '#e03e3e' }, { name: 'Suport telefon', mat: 'PETG Albastru', g: 22, dur: '55min', status: 'finalizat', color: '#3b82f6' }, { name: 'Cutie componente', mat: 'PLA+ Verde', g: 95, dur: '4h', status: 'in_curs', color: '#22c55e' }].map(p => (
        <MRow key={p.name}>
...
function MockOrders() {
  return (
    <Shell active="Comenzi">
      ...
      {[{ type: '📥', name: 'Bambu Lab', desc: '2× PLA 1kg', val: 178, status: 'livrata' }, { type: '📤', name: 'Ion Popescu', desc: 'Suport birou', val: 85, status: 'confirmata' }, { type: '📤', name: 'Maria Ionescu', desc: 'Figurine joc', val: 220, status: 'pending' }].map(o => (
        <MRow key={o.name}>
    ...
    <Shell active="Calculator">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 130px', gap: 12 }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {[{ icon: '🧵', label: 'Filament', rows: [['Bambu Lab PLA Roșu',''],['Grame folosite','48g']] }, { icon: '⚡', label: 'Energie', rows: [['Durată','135 min'],['Cost/oră','0.80 RON']] }, { icon: '🔧', label: 'Manoperă', rows: [['Timp','20 min'],['Tarif','50 RON/h']] }].map(sec => (
            <div key={sec.label} style={{ background: '#fff', borderRadius: 8, padding: '8px 10px', border: '1px solid #f1f5f9' }}>
              ...
              {sec.rows.filter(([,v]) => v).map(([k, v]) => (
                <div key={k} style={{ display: 'flex', justifyContent: 'space-between', fontSize: 9 }}><span style={{ color: '#94a3b8' }}>{k}</span><span style={{ fontWeight: 600 }}>{v}</span></div>
              ))}
            </div>
          ))}
        </div>
        <div style={{ background: '#0f172a', borderRadius: 10, padding: '14px 12px', display: 'flex', flexDirection: 'column', gap: 6 }}>
          ...
          {[['🧵','4.27'],['⚡','1.80'],['🔧','16.67']].map(([l,v]) => (
            <div key={l} style={{ display: 'flex', justifyContent: 'space-between', fontSize: 9 }}><span style={{ color: 'rgba(255,255,255,0.45)' }}>{l}</span><span style={{ color: '#fff', fontWeight: 600 }}>{v}</span></div>
          ))}
  ...
  return (
    <div style={{ background: '#fff', padding: '60px 0 0' }}>
      <div style={{ textAlign: 'center', padding: '0 20px 40px' }}>
        ...
        <h2 style={{ fontSize: 28, fontWeight: 800, letterSpacing: '-1px', margin: 0, color: '#0f172a' }}>Tot ce ai nevoie,<br />într-o singură aplicație.</h2>
      </div>
      {/* Tab pills */}
      <div style={{ display: 'flex', overflowX: 'auto', gap: 8, padding: '0 20px 20px', scrollbarWidth: 'none' }}>
        {SLIDES.map((s, i) => (
          <button key={s.tag} onClick={() => setActiveIndex(i)} style={{ flexShrink: 0, padding: '7px 14px', borderRadius: 99, border: 'none', cursor: 'pointer', fontSize: 12, fontWeight: 600, background: i === activeIndex ? slide.color : '#f1f5f9', color: i === activeIndex ? '#fff' : '#64748b', transition: 'all .2s' }}>{s.tag}</button>
        ))}
      </div>
      {/* Text */}
      <div style={{ padding: '0 20px 20px' }}>
        <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: '1.5px', textTransform: 'uppercase', color: slide.color, marginBottom: 8 }}>{slide.tag}</div>
        <h3 style={{ fontSize: 22, fontWeight: 800, letterSpacing: '-0.5px', lineHeight: 1.2, color: '#0f172a', margin: '0 0 10px' }}>{slide.title}</h3>
        <p style={{ fontSize: 14, lineHeight: 1.7, color: '#64748b', margin: 0 }}>{slide.desc}</p>
      </div>
      {/* Mock browser frame */}
      <div style={{ margin: '0 20px 60px', borderRadius: 14, overflow: 'hidden', border: '1px solid #e2e8f0', boxShadow: `0 12px 40px ${slide.color}20` }}>
        <div style={{ background: '#f8fafc', borderBottom: '1px solid #f1f5f9', padding: '8px 14px', display: 'flex', alignItems: 'center', gap: 5 }}>
          ...
          <div style={{ flex: 1, textAlign: 'center', fontSize: 10, color: '#94a3b8', fontFamily: 'monospace' }}>app.ro/dashboard</div>
        </div>
      ...
      <div style={{ position: 'sticky', top: 0, height: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#fff', overflow: 'hidden' }}>
        ...
        <div style={{ maxWidth: 1100, width: '100%', padding: '0 40px', display: 'grid', gridTemplateColumns: '1fr 1.5fr', gap: 60, alignItems: 'center' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            ...
            <h3 style={{ fontSize: 'clamp(22px,2.5vw,34px)', fontWeight: 800, letterSpacing: '-0.8px', lineHeight: 1.2, color: '#0f172a', margin: 0, opacity: entering ? 0 : 1, transform: entering ? 'translateY(12px)' : 'none', transition: 'all 0.35s ease 0.04s' }}>{slide.title}</h3>
            <p style={{ fontSize: 15, lineHeight: 1.75, color: '#64748b', margin: 0, opacity: entering ? 0 : 1, transform: entering ? 'translateY(10px)' : 'none', transition: 'all 0.35s ease 0.08s' }}>{slide.desc}</p>
            <div>
...
function Pricing({ width }) {
  ...
  return (
    <section id="pricing" ref={ref} style={{ padding: mob ? '70px 20px' : '100px 40px', background: '#fafafa' }}>
      <div style={{ maxWidth: 1100, margin: '0 auto' }}>
        <div style={{ textAlign: 'center', marginBottom: mob ? 40 : 60 }}>
          <h2 style={{ fontSize: mob ? 28 : 'clamp(28px,4vw,44px)', fontWeight: 800, letterSpacing: '-1px', lineHeight: 1.15, margin: '0 0 16px', opacity: inView ? 1 : 0, transform: inView ? 'none' : 'translateY(20px)', transition: 'all 0.6s ease' }}>
        ...
        <div style={{
          ...
          {PLANS.map((p, i) => (
            <div key={p.name} style={{
              ...
              borderRadius: 20, padding: mob ? '28px 24px' : '32px',
              position: 'relative',
              opacity: inView ? 1 : 0,
              ...
              {p.highlight && (
                <div style={{ position: 'absolute', top: -12, left: '50%', transform: 'translateX(-50%)', background: '#5c6ac4', color: '#fff', fontSize: 11, fontWeight: 700, padding: '4px 14px', borderRadius: 99, letterSpacing: '0.5px', whiteSpace: 'nowrap' }}>Popular</div>
              )}
              <div style={{ fontSize: 13, fontWeight: 700, letterSpacing: '1px', textTransform: 'uppercase', color: '#94a3b8', marginBottom: 16 }}>{p.name}</div>
              <div style={{ marginBottom: 28 }}>
                <span style={{ fontSize: 44, fontWeight: 800, letterSpacing: '-2px', color: p.highlight ? '#fff' : '#0f172a' }}>{p.price}</span>
                <span style={{ fontSize: 14, color: '#94a3b8' }}> {p.sub}</span>
              </div>
              <ul style={{ listStyle: 'none', padding: 0, margin: '0 0 32px', display: 'flex', flexDirection: 'column', gap: 12 }}>
                {p.features.map(f => (
                  <li key={f} style={{ fontSize: 14, color: p.highlight ? 'rgba(255,255,255,0.8)' : '#475569', display: 'flex', alignItems: 'center', gap: 8 }}>
                    <span style={{ color: p.highlight ? '#5c6ac4' : '#22c55e', fontWeight: 700 }}>✓</span> {f}
                  </li>
                ))}
              </ul>
              <a href="#" style={{
...
function Footer({ width }) {
  const mob = width < 640;
  return (
    <footer style={{ background: '#0f172a', color: '#fff', padding: mob ? '40px 20px' : '60px 40px' }}>
      <div style={{ maxWidth: 1100, margin: '0 auto', textAlign: 'center' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, marginBottom: 16 }}>
          <span style={{ width: 36, height: 36, background: '#5c6ac4', borderRadius: 9, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, fontSize: 16, flexShrink: 0 }}>3D</span>
        ...
        <div style={{ display: 'flex', justifyContent: 'center', gap: mob ? 16 : 24, marginBottom: 28, flexWrap: 'wrap' }}>
          {['Login', 'Register', 'Contact'].map(l => (
            <a key={l} href="#" style={{ fontSize: 13, color: 'rgba(255,255,255,0.5)', textDecoration: 'none' }}>{l}</a>
          ))}
        </div>
        <p style={{ fontSize: 11, color: 'rgba(255,255,255,0.25)', margin: 0 }}>© {new Date().getFullYear()} 3D•Print Studio. Toate drepturile rezervate.</p>
      </div>
    </footer>
  );
}
...
export function LandingPage() {
  ...
  return (
    <div style={{ fontFamily: "'DM Sans', 'Outfit', -apple-system, sans-serif", background: '#fafafa', color: '#0f172a', minHeight: '100vh', overflowX: 'clip' }}>
      <style>{`
